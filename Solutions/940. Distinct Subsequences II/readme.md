...
appeared earlier. `end[c]` stores the number of distinct subsequences that end
with character `c`. Therefore, for the current character `c`:
...
ending with the current character. Exactly `end[c]` of them were already present
as subsequences ending in `c`, so subtracting that value removes duplicates.
Adding `newSubsequences` to `end[c]` records every distinct subsequence ending
in `c`, preserving the invariant for the next occurrence.
